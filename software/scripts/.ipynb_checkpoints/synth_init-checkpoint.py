...
synth.set_RF_output(0,1,synth_opt,LOs) #Turn on the RF output. (device,state)
synth.set_RF_output(1,1,synth_opt,LOs)

synth.set_f(0,F,synth.synth_opt,LOs) #Set the frequency of the RF output, in MHz. (device, state) You must have the device's RF output in state (1) before doing this. 
synth.set_f(1,int(F+F_OFFSET),synth.synth_opt,LOs)
