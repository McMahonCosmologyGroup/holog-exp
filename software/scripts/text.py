...
nsamp = 5
arr2D_all_data=np.zeros((nsamp,(4*fpga_daq.RoachOpt.N_CHANNELS+5)))#, where the 7 extra are f,x,y,phi,... x_cur,y_cur, index_signal of peak cross power in a single bin (where phase is to be measured)
...
def during():
	...
	x_cur = positiion[0]
	y_cur = positiion[1]
	arr_aa, arr_bb, arr_ab, arr_phase,index_signal = fpga_daq.TakeAvgData(baseline,fpga)
	arr2D_all_data[ii] = ([f]+[x_cur]+[y_cur]+[phi]+[index_signal]+arr_aa.tolist()+arr_bb.tolist()+arr_ab.tolist()+arr_phase.tolist())
	ii+=1
	print(' end f = '+str(f))
